...
export class Pipeline {
  ...
  processDecodeStage() {
    ...
    // Special handling for the specific instruction format we have in memory
    // Check if this is the MOVI format (0x0600xxxxx)
    if ((instruction & 0xFF000000) === 0x06000000) {
      // This is a MOVI instruction with our specific format
      // Extract the register and immediate value
      const rd = (instruction >> 16) & 0xF; // Extract register from bits 16-19
      const imm = instruction & 0xFFFF;     // Extract immediate from bits 0-15
      
      console.log(`Decode: SPECIAL MOVI X${rd}, ${imm}`);
      
      const decodedInst = {
        type: 0,
        opcode: this.OPCODES.MOVI,
        rn: 0,
        rm: 0,
        rd: rd,
        imm: imm,
    ...
    // ADD instruction format (0x00140c00)
    if (instruction === 0x00140c00) {
      console.log(`Decode: SPECIAL ADD X3, X1, X2`);
    ...
    // SUB instruction format (0x01141000)
    if (instruction === 0x01141000) {
      console.log(`Decode: SPECIAL SUB X4, X1, X2`);
    ...
    // Default decoding for other instructions
      ...
      case 0: // ALU operations
        switch (inst.opcode) {
          ...
          case this.OPCODES.MOVI:
            // For MOVI, the immediate value is directly loaded
            result = inst.imm;
            console.log(`Execute: MOVI X${inst.rd} = ${inst.imm}`);
            break;
