...
function doFetchStage() {
    ...
    if (fetchSlotInst === null || (fetchSlotInst.isBubble && fetchSlotInst.completed)) {
        if (pipeline.squashFetchDecode) {
            const bubble = new PipelineInstruction(null, -1);
            bubble.isBubble = true; bubble.completed = true;
            pipeline.stages[STAGE_FETCH] = bubble;
            return;
        }
        const currentPC = PC.read();
        // Use actualProgramLength for accurate end-of-program detection
        if (currentPC >= pipeline.actualProgramLength && pipeline.actualProgramLength > 0) { // Only check if actualProgramLength is set
            console.log(`Cycle ${pipeline.clockCycle}: PC ${currentPC} reached end of loaded program (${pipeline.actualProgramLength}). Halting fetch.`);
        ...
        if (currentPC >= MAX_PROGRAM_SIZE) { // Fallback physical size check
             console.log(`Cycle ${pipeline.clockCycle}: PC ${currentPC} reached MAX_PROGRAM_SIZE (${MAX_PROGRAM_SIZE}). Halting fetch.`);
             pipeline.isHalted = true;
             const bubble = new PipelineInstruction(null, currentPC);
             bubble.isBubble = true; bubble.completed = true;
             pipeline.stages[STAGE_FETCH] = bubble;
        ...
        newInstruction.fetchInProgress = true;
        ...
        if (memReadStatus.status === 'error') {
            ...
            newInstruction.fetchInProgress = false; newInstruction.fetchError = memReadStatus.message;
            newInstruction.isBubble = true; pipeline.isHalted = true;
        }
        PC.write(currentPC + 1); // Tentatively increment PC
        ...
        if (fetchResult) {
            if (fetchResult.status === 'done') {
                fetchSlotInst.binaryInstruction = fetchResult.data;
                ...
                fetchSlotInst.fetchInProgress = false; fetchSlotInst.fetchError = fetchResult.message;
                fetchSlotInst.isBubble = true; pipeline.isHalted = true;
                memorySystem.clearRequestResult(`Workspace_inst_${fetchSlotInst.id}`);
            }
        } else if (!memorySystem.hasPendingRequest(`Workspace_inst_${fetchSlotInst.id}`) && fetchSlotInst.pc < (pipeline.actualProgramLength || MAX_PROGRAM_SIZE) && !memorySystem.isBusyForStage(`Workspace_inst_${fetchSlotInst.id}`)) {
...
function doDecodeStage() {
    ...
    if (inst.binaryInstruction === null || inst.binaryInstruction === undefined) {
        inst.isBubble = true; inst.opCode = "NOP"; return;
    }
    ...
    if (pipeline.squashFetchDecode) {
        ...
        pipeline.squashFetchDecode = false;
        return;
    }
    ...
    try {
        switch (typeBits) {
            ...
            case BIN_TYPE_CONTROL:
                ...
                else if (ctrlOp === 0b100) { inst.opCode = 'CAL'; inst.rn = (binary & MASK_CTRL_RN_BLT_CAL_FLUSH) >>> SHIFT_CTRL_RN_BLT_CAL_FLUSH; inst.immediate = signExtend(binary & MASK_CTRL_OFFSET22_BLT_CAL, 22); inst.hasWriteback = true; inst.rd = 31; }
        ...
        inst.opCode = "NOP_ERR_DEC"; inst.isBubble = true; pipeline.isHalted = true; return;
    }

    // Data Hazard Detection & Operand Fetch
    if (inst.type !== INSTRUCTION_TYPE_CONTROL_STR || ['JMP', 'BLT', 'CAL', 'FLUSH'].includes(inst.opCode) ) {
        let hazardDetected = false; const registersToRead = [];
        ...
        if (inst.opCode === 'STR') { if (inst.rd !== null && inst.rd !== undefined) registersToRead.push(inst.rd); }
        for (const regNum of registersToRead) {
            const exInst = pipeline.stages[STAGE_EXECUTE]; const memInst = pipeline.stages[STAGE_MEMORY];
            if (exInst && exInst.hasWriteback && exInst.rd === regNum && !exInst.isBubble && !exInst.completed) { hazardDetected = true; break; }
            if (memInst && memInst.hasWriteback && memInst.rd === regNum && !memInst.isBubble && !memInst.completed) { hazardDetected = true; break; }
        }
        if (hazardDetected) { inst.isStalled = true; return; }
        try {
            ...
            if (inst.opCode === 'STR') { if (inst.rd !== null && inst.rd !== undefined) inst.rdValueForStore = registers.read(inst.rd); }
        } catch (e) {
...
function advancePipelineStages() {
    ...
    if (memInst && !memInst.isStalled && !memInst.fetchInProgress && (memInst.isBubble || memInst.memOpCompleted || memInst.type !== INSTRUCTION_TYPE_MEMORY_STR || (memInst.type === INSTRUCTION_TYPE_CONTROL_STR && (memInst.branchTaken || memInst.opCode === 'FLUSH')))) {
        if (pipeline.stages[STAGE_WRITEBACK] === null) {
            pipeline.stages[STAGE_WRITEBACK] = memInst; pipeline.stages[STAGE_MEMORY] = null;
        }
    }
    const exInst = pipeline.stages[STAGE_EXECUTE];
    if (exInst && !exInst.isStalled && !exInst.fetchInProgress && !exInst.completed) {
        if (pipeline.stages[STAGE_MEMORY] === null) {
            pipeline.stages[STAGE_MEMORY] = exInst; pipeline.stages[STAGE_EXECUTE] = null;
        }
    }
    const decInst = pipeline.stages[STAGE_DECODE];
    if (decInst && !decInst.isStalled && !decInst.fetchInProgress && !decInst.completed) {
        if (pipeline.stages[STAGE_EXECUTE] === null) {
            pipeline.stages[STAGE_EXECUTE] = decInst; pipeline.stages[STAGE_DECODE] = null;
        }
    }
    ...
    if (fetchInst && !fetchInst.fetchInProgress && !fetchInst.completed) {
        if (pipeline.stages[STAGE_DECODE] === null) {
            ...
            if (!fetchInst.isBubble) { fetchInst.isStalled = true; }
        }
